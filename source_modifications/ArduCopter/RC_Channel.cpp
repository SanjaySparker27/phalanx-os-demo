...
{
    // Get RC channel 8 (index 7)
    RC_Channel *chan8 = rc().channel(7);
    if (chan8 == nullptr) {
        return;
    }
    
    // Get percentage input from slider (0-100)
    uint8_t slider_percent = chan8->percent_input();
    
    // Static variable to track last mode for debouncing
    static Mode::Number last_mode = Mode::Number::LOITER;
    static uint32_t last_check_ms = 0;
    
    uint32_t now_ms = AP_HAL::millis();
    
    // Check every 100ms for mode switching (10Hz)
    if (now_ms - last_check_ms < 100) {
        return;
    }
    last_check_ms = now_ms;
    
    Mode::Number desired_mode;
    
    // 0-16% slider = Loiter Mode (0-16%)
    // >16% slider = Stabilize Mode
    if (slider_percent <= 16) {
        desired_mode = Mode::Number::LOITER;
    } else {
        desired_mode = Mode::Number::STABILIZE;
    }
    ...
    if (desired_mode != last_mode) {
        if (copter.set_mode(desired_mode, ModeReason::RC_COMMAND)) {
            ...
            gcs().send_text(MAV_SEVERITY_INFO, "Mode changed via slider: %s (%u%%)", 
